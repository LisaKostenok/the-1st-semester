...
//! @param[in] char* buffer - address of array of all the symbols from text
//!
//---------------------------------------------------------------------------------------------------------------------

void cleaner(char** copy, char** arrpointers, char* buffer);
...
int main()
{
    ...
    cleaner(copy, arrpointers, arrpointers[0]);
}
...
char** readtext(unsigned int* nlines, unsigned int* nsymb)
{
    ...
    char** buf = makelines(buffer, nlines, *nsymb);
